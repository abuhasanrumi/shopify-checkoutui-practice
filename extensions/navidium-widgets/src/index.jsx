        ...
        {/* heading and buttons */}
        {
          JSON.parse(data?.widgets?.at(0)?.widget_customization)?.scroll_button_visibility ||
            ...
            <Grid columns={['fill', 'auto']}>
              {
                JSON.parse(data?.widgets.at(0)?.widget_customization)?.title_visibility ?
                  <Heading level={3}>{JSON.parse(data?.widgets?.at(0)?.widget_customization)?.title || 'You may also like'}</Heading>
                  : null
              }
              {
                JSON.parse(data?.widgets?.at(0)?.widget_customization)?.scroll_button_visibility ?
...
function SMS({ data }) {
  ...
  return (
    <BlockLayout border='base' padding='base' cornerRadius='base'>
      ...
      <Text appearance="subdued">{widgetData?.title || 'Stay up to date on your purchase with order confirmation and shipping confirmation messages.'}</Text>
    </BlockLayout>
  )
}
  ...
  return (
    <>
      <BlockLayout rows={['auto', 'fill']} border='base' borderWidth='base' borderRadius='base' padding='base' spacing='base'>
        {/* text */}
        <TextBlock inlineAlignment="center">{widgetData?.title || 'Guaranteed Safe & Secure Checkout'}</TextBlock>
...
function FAQ({ data }) {
  ...
  return (
    <>
      <Heading level={2}>{faqTitle || 'Frequently asked questions'}</Heading>
      <BlockSpacer />
      <Divider size="base" />
      <BlockSpacer />
      {
...
function Quote({ data }) {
  ...
  return (
    <>
      <BlockStack border='base' borderRadius='base' borderWidth='base' padding='base'>
        <Text size='base'>{widgetData?.title}</Text>
        <Text size='base' appearance='subdued'>― Buy now</Text>
        <Grid columns={['10%', 'fill', '30%']} spacing='base' blockAlignment='center'>
          <Image source={widgetData?.user?.image} borderRadius='base' />
          <Text>{widgetData?.user?.name}</Text>
          <InlineStack inlineAlignment='end'>
