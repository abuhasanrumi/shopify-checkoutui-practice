                ...
                JSON.parse(data?.widgets?.at(0)?.widget_customization)?.scroll_button_visibility ?
                  <InlineStack inlineAlignment='end'>
                    <Button
                      ...
                      onPress={() => {
                        setSkeleton(true)
                        setNext(state => next === 0 ? state : state - productOnScreen)
                      }} kind='plain'><Icon source='chevronLeft' /></Button>
                    <Button
                      disabled={products.length - next <= productOnScreen}
                      onPress={() => {
                        setSkeleton(true)
                        setNext(state => products.length - next > productOnScreen ? state + productOnScreen : state + products.length - next)
